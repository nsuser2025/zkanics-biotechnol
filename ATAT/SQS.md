...
<p>
mcsqs コードは MPI（メッセージ・パッシング・インターフェース）をサポートしておらず、単一の CPU 上で動作します。
しかし、-ip=xx というフラグを使って、手動でランダムシード（乱数の種）を与えることができます。
これを利用して、異なるランダムシードを設定した複数の mcsqs インスタンスを独立して同時に実行させることが可能です。
これにより、目的の SQS をより早く見つけられるようになります。
すべての計算が終わった後で、最も良い相関関数の結果を持つものを選択すればよいのです。
また別の方法として、インスタンスごとに異なるセル形状（sqscell.out）を指定して
-rc フラグで実行するという手段もあります。
mcsqs のジョブを『リスタート』することはできません。
また、リスタートする意味もありません。
なぜなら、SQSの生成は構造をランダムに探索するモンテカルロ法に基づいているからです。
例えば、ある mcsqs インスタンスが目的関数 $-0.8$ で動いていて、
私たちが目的関数 $-0.9$ の SQS を見つけたいとしましょう。
このとき、実行中のジョブを継続するのも、新しいジョブを一から始めるのも、
目的の SQS が見つかる確率は同じなのです。  
</p>

<p>
実践的なポイントの解説
1. 「力技」の並列化（Embarrassingly Parallel）mcsqs 自体に計算を分割する機能（MPI）はありませんが、
複数のプロセスを同時に立ち上げることで実質的な並列化が可能です。
方法: -ip の値を 1, 2, 3... と変えて、スパコンの各ノードやPCの各コアで一斉に走らせます。
メリット: 1つのジョブで1ヶ月待つ代わりに、30個のジョブを1日走らせて、
その中の「最高傑作」を拾うという戦略が取れます。</br>
2. なぜ「リスタート」が不要なのか?
通常の第一原理計算（VASPなど）では、計算が止まったらその時点の座標から再開するのが常識ですが、mcsqs は異なります。
理由: モンテカルロ探索は、現在の状態から次の状態をランダムに探すプロセスであり、
過去の履歴に依存しません（マルコフ性）。
考え方: すでに良いスコア（$-0.8$）に達しているインスタンスであっても、
次の瞬間に $-0.9$ が見つかる確率は、別の場所からスタートしたインスタンスが $-0.9$ を見つける確率と変わらない、
という数学的な性質を指しています。</br>
