...
SQS に含まれる原子数は, rndstr.in で定義されたサイト数の倍数で,
なおかつ全ての元素の原子数が整数になるように設定する.
例えば, 三元系の等原子組成 L1${_2}$ 合金の場合,
SQS には少なくとも 12 個の原子が含まれている必要がある.
mcsqs は, 系の複雑さやカットオフの設定次第で無限に走り続けることもある.
通常, 第 2 近接の相関関数の誤差が大きいときは,
より外側のシェルの誤差を圧倒することが多いので,
カットオフには第 3 近接以上を含める必要はほとんどない.
二元系合金の SQS は数秒で探索できるが, 五元系ランダム合金では,
目的関数の値が十分小さい SQS を見つけるだけでも数日から数週間かかることもある.
どんな構造を SQS として取り出すかの明確な基準はないが,
格子ベクトルが非常に長く, ベクトル間の角度が非常に小さいような SQS は DFT
